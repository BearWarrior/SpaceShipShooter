...
public class ChassisPlayer : Chassis 
{
	...
	void Awake() 
	{
		...
		//On crée un autre Transform pour stocker l'instantiation 
		Transform chassisCree;
		...
		chassisCree.transform.localPosition = new Vector3 (0, 0, 0);	

		chassisCree.tag = "ChassiPlayer";
		...
		if (nom == "Chassi1") 
		{
			...
			{
				chassiActuel.posArme.Add(Vector3.zero);
			...
			{
				chassiActuel.posReacteur.Add(Vector3.zero);
	...
	void Update () 
	{
		...
		{
			//Si on n'est pas en mode flotte
			if( !GameObject.Find ("patternToLaunch").GetComponent<PatternToLaunch> ().flotte )
			{
				if(GameObject.FindWithTag("Player").GetComponent<characterController>().shootOK)
				{
				//On tir seulement si le menu pause n'est pas affiché (et existe)
					if(GameObject.FindWithTag("Pause") != null)
					{
						if(GameObject.FindWithTag("Pause").GetComponent<MenuPause>().pause == false)
						{
							...
							for(int i = 0; i < chassiActuel.nbPointDattache; i++)
							{
								posArmeGlobal = posArme[i] + new Vector3(GameObject.FindWithTag("Player").transform.position.x, GameObject.FindWithTag("Player").transform.position.y, GameObject.FindWithTag("Player").transform.position.z);
	...
	private void gestionFumeeDegats()
	{
		...
		if(fumeeAff[0] && fumeeT[0] == null) //On affiche la fumee 0 ssi fummeAff[0] = true et fumeeT n'existe pas
		{
			...
			fumeeT[0].parent = GameObject.FindWithTag("Player").transform;
		...
		if(fumeeAff[1] && fumeeT[1] == null) //On affiche la fumee 0 ssi fummeAff[0] = true et fumeeT n'existe pas
		{
			...
			fumeeT[1].parent = GameObject.FindWithTag("Player").transform;
